                                ...
                                <?php foreach ($child_data as $key => $task):?>
                                    <tr role="row" class="<?=$key % 2 ? 'odd' : 'even'?>">
                                        ...
                                        <td>
                                            ...
                                            <a class="btn btn-danger btn-xs" href="<?=url('/admin/task/destroy', $task['id'])?>"><i class="fa fa-trash"></i></a>

                                            <?php if(isset($_SESSION['user']) && !$task['done']):?>
                                                <a title="Mark done" class="btn btn-success btn-xs" href="<?=url('/admin/task/done', $task['id'])?>">Mark Complete</a>
                                            <?php elseif (isset($_SESSION['user']) && $task['done']):?>
                                                <a title="Mark incomplete" class="btn btn-warning btn-xs" href="<?=url('/admin/task/incomplete', $task['id'])?>">Mark Incomplete</a>
                                            <?php else:?>
